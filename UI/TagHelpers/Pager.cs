...
public class Pager(IHttpContextAccessor httpContextAccessor, LinkGenerator linkGenerator) : TagHelper
{
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
        ...
        if (currentPage == 1) prevLink.AddCssClass("disabled");
        prevLink.MergeAttribute("href",
            linkGenerator.GetPathByAction(httpContextAccessor.HttpContext!, "Index", "Games",
                new { pageNumber = currentPage - 1, gameGenreId = selectedGameGenreId }));
        ...
        for (var pageNum = 1; pageNum <= totalPages; pageNum++)
        {
            ...
            if (currentPage == pageNum) pageLink.AddCssClass("disabled");
            pageLink.MergeAttribute("href",
                linkGenerator.GetPathByAction(httpContextAccessor.HttpContext!, "Index", "Games",
                    new { pageNumber = pageNum, gameGenreId = selectedGameGenreId }));
        ...
        if (currentPage == totalPages) nextLink.AddCssClass("disabled");
        nextLink.MergeAttribute("href",
            linkGenerator.GetPathByAction(httpContextAccessor.HttpContext!, "Index", "Games",
                new { pageNumber = currentPage + 1, gameGenreId = selectedGameGenreId }));
