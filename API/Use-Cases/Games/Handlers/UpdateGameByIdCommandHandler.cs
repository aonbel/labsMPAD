...
public sealed class UpdateGameByIdCommandHandler(AppDbContext context) : IRequestHandler<UpdateGameByIdCommand, ResponseData<Game>>
{
    public async Task<ResponseData<Game>> Handle(UpdateGameByIdCommand request, CancellationToken cancellationToken)
    {
        ...
        if (game is null)
        {
            return ResponseData<Game>.Fail($"Game with id {request.Game.Id} does not exist");
        }
